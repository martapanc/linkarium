...
export function LinkCard({ link, index, onDelete, onRescrape }: Props) {
  ...
  const hasImage = link.image_url && !imgError;
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-[15px] font-medium text-sand-900 truncate leading-snug">
            {link.title || link.url}
          </h3>
          {link.description && (
          ...
          <>
            ...
            <div className="absolute right-0 top-full mt-1 z-20 bg-white rounded-lg border border-sand-200 shadow-lg py-1 min-w-[160px]">
              <button
                ...
                className="w-full text-left px-3 py-2 text-sm text-sand-700 hover:bg-sand-50 flex items-center gap-2 cursor-pointer"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              >
                <svg
                  className={`w-4 h-4 ${isRescraping ? "animate-spin" : ""}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
